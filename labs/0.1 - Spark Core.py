...
customer_summary_df = customers_df.select("customerID", "first_name", "last_name", "city"
)
...
visa_transactions_df = transactions_df.filter( transactions_df.paymentMethod == "visa" )
...
transactions_with_customers_df = (transactions_df
    .join(
        customers_df,transactions_df["customerID"] == customers_df["customerID"],"inner"
          # Join condition here
...
customers_without_reviews_df = (customers_df
    .join(
        reviews_df,
        customers_df["customerID"] == reviews_df["franchiseID"], "left" # Note: This is a simplified join
...
franchise_metrics_df = (transactions_df
    ...
    # 1. Add payment category using existing UDF
    .withColumn(
        "payment_category",
        categorize_payment_udf(col("paymentMethod"))
    )
    ...
    # 3. Group by franchiseID and aggregate metrics
    .groupBy("franchiseID")
    .agg(
    ...
    # 4. Sort by total revenue and take top 5
