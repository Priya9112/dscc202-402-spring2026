...
enhanced_df = base_df.withColumn(
    ...
    round((unix_timestamp(col("tpep_dropoff_datetime")) - 
         unix_timestamp(col("tpep_pickup_datetime"))) / 60, 2)  # Dropoff and pickup columns
).withColumn(
    "avg_speed_mph",
...
# MAGIC   AND hour(tpep_pickup_datetime) BETWEEN 17 AND 19;
...
# MAGIC WHERE fare_amount <= 0
# MAGIC    OR trip_distance < 0
# MAGIC    OR trip_duration_minutes <= 0;
...
# MAGIC
# MAGIC VACUUM delta.`/Volumes/nyctaxi_catalog/analytics/workspace/taxi_trips_delta`
# MAGIC RETAIN 168 HOURS DRY RUN;
