...
hourly_sales_df = (streaming_df
    ...
    .agg(
        sum("franchiseID").alias("total_sales"),  # Column to sum
...
hourly_query = (hourly_sales_df
    ...
    .option("checkpointLocation", f"{working_dir}/hourly_verification")  # Checkpoint path
...
query = (hourly_sales_df
    ...
    .option("checkpointLocation", f"{checkpoint_dir}/hourly_sales_checkpoint")  # Checkpoint path
...
from pyspark.sql.functions import current_timestamp, sum, count, approx_count_distinct, avg, window, col

# Create streaming source with watermark
streaming_df = (spark.readStream
    .format("delta")  # Delta format
    .load(f"{working_dir}/streaming_source")  # Path to streaming source
    .withWatermark("dateTime", "10 minutes")  # Watermark
)

# Metric 1: Running total sales by franchise
running_total_df = (streaming_df
    .groupBy("franchiseID")  # Group by franchise
    .agg(
