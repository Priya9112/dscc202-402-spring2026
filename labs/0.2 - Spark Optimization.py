...
# DBTITLE 1,Task 1.2: Demonstrate Predicate Pushdown with Partitioned Delta
...
clean_customers_df = spark.read.format("delta").load(
    f"{working_dir}/customers_deduplicated"  # Load from f"{working_dir}/customers_deduplicated"
)
# Step 2: Load and filter franchises to US only (predicate pushdown!)
usa_franchises_df = spark.table("samples.bakehouse.sales_franchises") \
    .filter(col("country") == "US")  # Load franchises and filter country == "USA"
